...
def formatVitalsStg1(con, schemaName):
    ...
    dataVitals1Query = """
        ...
        group by vit.visit_occurrence_id, vit.rn
        ;
        """
...
def formatVitalsMax(con, schemaName):
    ...
    dataVitalsMaxQuery = """
        ...
        distinct
        vit.visit_occurrence_id as visit_occurrence_id
        , max(vit.tmp) over (partition by vit.visit_occurrence_id) as temp_max
        , max(vit.heartrate) over (partition by vit.visit_occurrence_id) as heartrate_max
        , max(vit.breath_rate_vent) over (partition by vit.visit_occurrence_id) as breath_rate_vent_max
        , max(vit.breath_rate_spon) over (partition by vit.visit_occurrence_id) as breath_rate_spon_max
        , max(vit.resp_rate) over (partition by vit.visit_occurrence_id) as resp_rate_max
        , max(vit.oxygen) over (partition by vit.visit_occurrence_id) as oxygen_max
        , max(vit.sysbp) over (partition by vit.visit_occurrence_id) as sysbp_max
        , max(vit.diabp) over (partition by vit.visit_occurrence_id) as diabp_max
        , max(vit.meanbp) over (partition by vit.visit_occurrence_id) as meanbp_max
        , max(vit.sysbp_ni) over (partition by vit.visit_occurrence_id) as sysbp_ni_max
        , max(vit.diabp_ni) over (partition by vit.visit_occurrence_id) as diabp_ni_max
        , max(vit.meanbp_ni) over (partition by vit.visit_occurrence_id) as meanbp_ni_max
        , max(vit.gcs_motor) over (partition by vit.visit_occurrence_id) as gcs_motor_max
        , max(vit.gcs_verbal) over (partition by vit.visit_occurrence_id) as gcs_verbal_max
        , max(vit.gcs_eye) over (partition by vit.visit_occurrence_id) as gcs_eye_max
...
def formatVitalsMin(con, schemaName):
    ...
    dataVitalsMinQuery = """
        ...
        distinct
        vit.visit_occurrence_id as visit_occurrence_id
        , min(vit.tmp) over (partition by vit.visit_occurrence_id) as temp_min
        , min(vit.heartrate) over (partition by vit.visit_occurrence_id) as heartrate_min
        , min(vit.breath_rate_vent) over (partition by vit.visit_occurrence_id) as breath_rate_vent_min
        , min(vit.breath_rate_spon) over (partition by vit.visit_occurrence_id) as breath_rate_spon_min
        , min(vit.resp_rate) over (partition by vit.visit_occurrence_id) as resp_rate_min
        , min(vit.oxygen) over (partition by vit.visit_occurrence_id) as oxygen_min
        , min(vit.sysbp) over (partition by vit.visit_occurrence_id) as sysbp_min
        , min(vit.diabp) over (partition by vit.visit_occurrence_id) as diabp_min
        , min(vit.meanbp) over (partition by vit.visit_occurrence_id) as meanbp_min
        , min(vit.sysbp_ni) over (partition by vit.visit_occurrence_id) as sysbp_ni_min
        , min(vit.diabp_ni) over (partition by vit.visit_occurrence_id) as diabp_ni_min
        , min(vit.meanbp_ni) over (partition by vit.visit_occurrence_id) as meanbp_ni_min
        , min(vit.gcs_motor) over (partition by vit.visit_occurrence_id) as gcs_motor_min
        , min(vit.gcs_verbal) over (partition by vit.visit_occurrence_id) as gcs_verbal_min
        , min(vit.gcs_eye) over (partition by vit.visit_occurrence_id) as gcs_eye_min
        from """ + schemaName + """.vitals_stg_1 vit
        ;
    """
...
def formatVitalsAvg(con, schemaName):
    ...
    dataVitalsAvgQuery = """
        ...
        distinct
        vit.visit_occurrence_id as visit_occurrence_id
        , avg(vit.tmp) over (partition by vit.visit_occurrence_id) as temp_avg
        , avg(vit.heartrate) over (partition by vit.visit_occurrence_id) as heartrate_avg
        , avg(vit.breath_rate_vent) over (partition by vit.visit_occurrence_id) as breath_rate_vent_avg
        , avg(vit.breath_rate_spon) over (partition by vit.visit_occurrence_id) as breath_rate_spon_avg
        , avg(vit.resp_rate) over (partition by vit.visit_occurrence_id) as resp_rate_avg
        , avg(vit.oxygen) over (partition by vit.visit_occurrence_id) as oxygen_avg
        , avg(vit.sysbp) over (partition by vit.visit_occurrence_id) as sysbp_avg
        , avg(vit.diabp) over (partition by vit.visit_occurrence_id) as diabp_avg
        , avg(vit.meanbp) over (partition by vit.visit_occurrence_id) as meanbp_avg
        , avg(vit.sysbp_ni) over (partition by vit.visit_occurrence_id) as sysbp_ni_avg
        , avg(vit.diabp_ni) over (partition by vit.visit_occurrence_id) as diabp_ni_avg
        , avg(vit.meanbp_ni) over (partition by vit.visit_occurrence_id) as meanbp_ni_avg
        , avg(vit.gcs_motor) over (partition by vit.visit_occurrence_id) as gcs_motor_avg
        , avg(vit.gcs_verbal) over (partition by vit.visit_occurrence_id) as gcs_verbal_avg
        , avg(vit.gcs_eye) over (partition by vit.visit_occurrence_id) as gcs_eye_avg
        from """ + schemaName + """.vitals_stg_1 vit
        ;
    """
...
def formatVitalsStd(con, schemaName):
    ...
    dataVitalsStdQuery = """
        ...
        distinct
        vit.visit_occurrence_id as visit_occurrence_id
        , stddev(vit.tmp) over (partition by vit.visit_occurrence_id) as temp_sd
        , stddev(vit.heartrate) over (partition by vit.visit_occurrence_id) as heartrate_sd
        , stddev(vit.breath_rate_vent) over (partition by vit.visit_occurrence_id) as breath_rate_vent_sd
        , stddev(vit.breath_rate_spon) over (partition by vit.visit_occurrence_id) as breath_rate_spon_sd
        , stddev(vit.resp_rate) over (partition by vit.visit_occurrence_id) as resp_rate_sd
        , stddev(vit.oxygen) over (partition by vit.visit_occurrence_id) as oxygen_sd
        , stddev(vit.sysbp) over (partition by vit.visit_occurrence_id) as sysbp_sd
        , stddev(vit.diabp) over (partition by vit.visit_occurrence_id) as diabp_sd
        , stddev(vit.meanbp) over (partition by vit.visit_occurrence_id) as meanbp_sd
        , stddev(vit.sysbp_ni) over (partition by vit.visit_occurrence_id) as sysbp_ni_sd
        , stddev(vit.diabp_ni) over (partition by vit.visit_occurrence_id) as diabp_ni_sd
        , stddev(vit.meanbp_ni) over (partition by vit.visit_occurrence_id) as meanbp_ni_sd
        , stddev(vit.gcs_motor) over (partition by vit.visit_occurrence_id) as gcs_motor_sd
        , stddev(vit.gcs_verbal) over (partition by vit.visit_occurrence_id) as gcs_verbal_sd
        , stddev(vit.gcs_eye) over (partition by vit.visit_occurrence_id) as gcs_eye_sd
        from """ + schemaName + """.vitals_stg_1 vit
        ;
    """
...
def formatVitalsFirst(con, schemaName):
    ...
    dataVitalsFirstQuery = """
        ...
        distinct
        vit.visit_occurrence_id as visit_occurrence_id
        , first_value(vit.tmp) over (partition by vit.visit_occurrence_id order by case when vit.tmp is not null then 0 else 1 end asc, vit.rn asc) as temp_first
        , first_value(vit.heartrate) over (partition by vit.visit_occurrence_id order by case when vit.heartrate is not null then 0 else 1 end asc, vit.rn asc) as heartrate_first
        , first_value(vit.breath_rate_vent) over (partition by vit.visit_occurrence_id order by case when vit.breath_rate_vent is not null then 0 else 1 end asc, vit.rn asc) as breath_rate_vent_first
        , first_value(vit.breath_rate_spon) over (partition by vit.visit_occurrence_id order by case when vit.breath_rate_spon is not null then 0 else 1 end asc, vit.rn asc) as breath_rate_spon_first
        , first_value(vit.resp_rate) over (partition by vit.visit_occurrence_id order by case when vit.resp_rate is not null then 0 else 1 end asc, vit.rn asc) as resp_rate_first
        , first_value(vit.oxygen) over (partition by vit.visit_occurrence_id order by case when vit.oxygen is not null then 0 else 1 end asc, vit.rn asc) as oxygen_first
        , first_value(vit.sysbp) over (partition by vit.visit_occurrence_id order by case when vit.sysbp is not null then 0 else 1 end asc, vit.rn asc) as sysbp_first
        , first_value(vit.diabp) over (partition by vit.visit_occurrence_id order by case when vit.diabp is not null then 0 else 1 end asc, vit.rn asc) as diabp_first
        , first_value(vit.meanbp) over (partition by vit.visit_occurrence_id order by case when vit.meanbp is not null then 0 else 1 end asc, vit.rn asc) as meanbp_first
        , first_value(vit.sysbp_ni) over (partition by vit.visit_occurrence_id order by case when vit.sysbp_ni is not null then 0 else 1 end asc, vit.rn asc) as sysbp_ni_first
        , first_value(vit.diabp_ni) over (partition by vit.visit_occurrence_id order by case when vit.diabp_ni is not null then 0 else 1 end asc, vit.rn asc) as diabp_ni_first
        , first_value(vit.meanbp_ni) over (partition by vit.visit_occurrence_id order by case when vit.meanbp_ni is not null then 0 else 1 end asc, vit.rn asc) as meanbp_ni_first
        , first_value(vit.gcs_motor) over (partition by vit.visit_occurrence_id order by case when vit.gcs_motor is not null then 0 else 1 end asc, vit.rn asc) as gcs_motor_first
        , first_value(vit.gcs_verbal) over (partition by vit.visit_occurrence_id order by case when vit.gcs_verbal is not null then 0 else 1 end asc, vit.rn asc) as gcs_verbal_first
        , first_value(vit.gcs_eye) over (partition by vit.visit_occurrence_id order by case when vit.gcs_eye is not null then 0 else 1 end asc, vit.rn asc) as gcs_eye_first
        from """ + schemaName + """.vitals_stg_1 vit
        ;
    """
...
def formatVitalsLast(con, schemaName):
    ...
    dataVitalsLastQuery = """
        ...
        distinct
        vit.visit_occurrence_id as visit_occurrence_id
        , first_value(vit.tmp) over (partition by vit.visit_occurrence_id order by case when vit.tmp is not null then 0 else 1 end asc, vit.rn desc) as temp_last
        , first_value(vit.heartrate) over (partition by vit.visit_occurrence_id order by case when vit.heartrate is not null then 0 else 1 end asc, vit.rn desc) as heartrate_last
        , first_value(vit.breath_rate_vent) over (partition by vit.visit_occurrence_id order by case when vit.breath_rate_vent is not null then 0 else 1 end asc, vit.rn desc) as breath_rate_vent_last
        , first_value(vit.breath_rate_spon) over (partition by vit.visit_occurrence_id order by case when vit.breath_rate_spon is not null then 0 else 1 end asc, vit.rn desc) as breath_rate_spon_last
        , first_value(vit.resp_rate) over (partition by vit.visit_occurrence_id order by case when vit.resp_rate is not null then 0 else 1 end asc, vit.rn desc) as resp_rate_last
        , first_value(vit.oxygen) over (partition by vit.visit_occurrence_id order by case when vit.oxygen is not null then 0 else 1 end asc, vit.rn desc) as oxygen_last
        , first_value(vit.sysbp) over (partition by vit.visit_occurrence_id order by case when vit.sysbp is not null then 0 else 1 end asc, vit.rn desc) as sysbp_last
        , first_value(vit.diabp) over (partition by vit.visit_occurrence_id order by case when vit.diabp is not null then 0 else 1 end asc, vit.rn desc) as diabp_last
        , first_value(vit.meanbp) over (partition by vit.visit_occurrence_id order by case when vit.meanbp is not null then 0 else 1 end asc, vit.rn desc) as meanbp_last
        , first_value(vit.sysbp_ni) over (partition by vit.visit_occurrence_id order by case when vit.sysbp_ni is not null then 0 else 1 end asc, vit.rn desc) as sysbp_ni_last
        , first_value(vit.diabp_ni) over (partition by vit.visit_occurrence_id order by case when vit.diabp_ni is not null then 0 else 1 end asc, vit.rn desc) as diabp_ni_last
        , first_value(vit.meanbp_ni) over (partition by vit.visit_occurrence_id order by case when vit.meanbp_ni is not null then 0 else 1 end asc, vit.rn desc) as meanbp_ni_last
        , first_value(vit.gcs_motor) over (partition by vit.visit_occurrence_id order by case when vit.gcs_motor is not null then 0 else 1 end asc, vit.rn desc) as gcs_motor_last
        , first_value(vit.gcs_verbal) over (partition by vit.visit_occurrence_id order by case when vit.gcs_verbal is not null then 0 else 1 end asc, vit.rn desc) as gcs_verbal_last
        , first_value(vit.gcs_eye) over (partition by vit.visit_occurrence_id order by case when vit.gcs_eye is not null then 0 else 1 end asc, vit.rn desc) as gcs_eye_last
        from """ + schemaName + """.vitals_stg_1 vit
        ;
    """
...
def formatLabsStg1(con, schemaName):
    ...
    dataLabs1Query = """
        ...
        group by lab.visit_occurrence_id, lab.rn
        ;
        """
...
def formatLabsMax(con, schemaName):
    ...
    dataLabsMaxQuery = """
        ...
        distinct
        lab.visit_occurrence_id as visit_occurrence_id
        , max(lab.potassium) over (partition by lab.visit_occurrence_id) as potassium_max
        , max(lab.chloride) over (partition by lab.visit_occurrence_id) as chloride_max
        , max(lab.glucose) over (partition by lab.visit_occurrence_id) as glucose_max
        , max(lab.sodium) over (partition by lab.visit_occurrence_id) as sodium_max
        , max(lab.bicarbonate) over (partition by lab.visit_occurrence_id) as bicarbonate_max
        , max(lab.hemoglobin) over (partition by lab.visit_occurrence_id) as hemoglobin_max
        , max(lab.creatinine) over (partition by lab.visit_occurrence_id) as creatinine_max
...
def formatLabsMin(con, schemaName):
    ...
    dataLabsMinQuery = """
        ...
        distinct
        lab.visit_occurrence_id as visit_occurrence_id
        , min(lab.potassium) over (partition by lab.visit_occurrence_id) as potassium_min
        , min(lab.chloride) over (partition by lab.visit_occurrence_id) as chloride_min
        , min(lab.glucose) over (partition by lab.visit_occurrence_id) as glucose_min
        , min(lab.sodium) over (partition by lab.visit_occurrence_id) as sodium_min
        , min(lab.bicarbonate) over (partition by lab.visit_occurrence_id) as bicarbonate_min
        , min(lab.hemoglobin) over (partition by lab.visit_occurrence_id) as hemoglobin_min
        , min(lab.creatinine) over (partition by lab.visit_occurrence_id) as creatinine_min
        from """ + schemaName + """.labs_stg_1 lab
        ;
    """
...
def formatLabsAvg(con, schemaName):
    ...
    dataLabsAvgQuery = """
        ...
        distinct
        lab.visit_occurrence_id as visit_occurrence_id
        , avg(lab.potassium) over (partition by lab.visit_occurrence_id) as potassium_avg
        , avg(lab.chloride) over (partition by lab.visit_occurrence_id) as chloride_avg
        , avg(lab.glucose) over (partition by lab.visit_occurrence_id) as glucose_avg
        , avg(lab.sodium) over (partition by lab.visit_occurrence_id) as sodium_avg
        , avg(lab.bicarbonate) over (partition by lab.visit_occurrence_id) as bicarbonate_avg
        , avg(lab.hemoglobin) over (partition by lab.visit_occurrence_id) as hemoglobin_avg
        , avg(lab.creatinine) over (partition by lab.visit_occurrence_id) as creatinine_avg
        from """ + schemaName + """.labs_stg_1 lab
        ;
    """
...
def formatLabsStd(con, schemaName):
    ...
    dataLabsStdQuery = """
        ...
        distinct
        lab.visit_occurrence_id as visit_occurrence_id
        , stddev(lab.potassium) over (partition by lab.visit_occurrence_id) as potassium_sd
        , stddev(lab.chloride) over (partition by lab.visit_occurrence_id) as chloride_sd
        , stddev(lab.glucose) over (partition by lab.visit_occurrence_id) as glucose_sd
        , stddev(lab.sodium) over (partition by lab.visit_occurrence_id) as sodium_sd
        , stddev(lab.bicarbonate) over (partition by lab.visit_occurrence_id) as bicarbonate_sd
        , stddev(lab.hemoglobin) over (partition by lab.visit_occurrence_id) as hemoglobin_sd
        , stddev(lab.creatinine) over (partition by lab.visit_occurrence_id) as creatinine_sd
        from """ + schemaName + """.labs_stg_1 lab
        ;
    """
...
def formatLabsFirst(con, schemaName):
    ...
    dataLabsFirstQuery = """
        ...
        distinct
        lab.visit_occurrence_id as visit_occurrence_id
        , first_value(lab.potassium) over (partition by lab.visit_occurrence_id order by case when lab.potassium is not null then 0 else 1 end asc, lab.rn asc) as potassium_first
        , first_value(lab.chloride) over (partition by lab.visit_occurrence_id order by case when lab.chloride is not null then 0 else 1 end asc, lab.rn asc) as chloride_first
        , first_value(lab.glucose) over (partition by lab.visit_occurrence_id order by case when lab.glucose is not null then 0 else 1 end asc, lab.rn asc) as glucose_first
        , first_value(lab.sodium) over (partition by lab.visit_occurrence_id order by case when lab.sodium is not null then 0 else 1 end asc, lab.rn asc) as sodium_first
        , first_value(lab.bicarbonate) over (partition by lab.visit_occurrence_id order by case when lab.bicarbonate is not null then 0 else 1 end asc, lab.rn asc) as bicarbonate_first
        , first_value(lab.hemoglobin) over (partition by lab.visit_occurrence_id order by case when lab.hemoglobin is not null then 0 else 1 end asc, lab.rn asc) as hemoglobin_first
        , first_value(lab.creatinine) over (partition by lab.visit_occurrence_id order by case when lab.creatinine is not null then 0 else 1 end asc, lab.rn asc) as creatinine_first
        from """ + schemaName + """.labs_stg_1 lab
        ;
    """
...
def formatLabsLast(con, schemaName):
    ...
    dataLabsLastQuery = """
        ...
        distinct
        lab.visit_occurrence_id as visit_occurrence_id
        , first_value(lab.potassium) over (partition by lab.visit_occurrence_id order by case when lab.potassium is not null then 0 else 1 end asc, lab.rn desc) as potassium_last
        , first_value(lab.chloride) over (partition by lab.visit_occurrence_id order by case when lab.chloride is not null then 0 else 1 end asc, lab.rn desc) as chloride_last
        , first_value(lab.glucose) over (partition by lab.visit_occurrence_id order by case when lab.glucose is not null then 0 else 1 end asc, lab.rn desc) as glucose_last
        , first_value(lab.sodium) over (partition by lab.visit_occurrence_id order by case when lab.sodium is not null then 0 else 1 end asc, lab.rn desc) as sodium_last
        , first_value(lab.bicarbonate) over (partition by lab.visit_occurrence_id order by case when lab.bicarbonate is not null then 0 else 1 end asc, lab.rn desc) as bicarbonate_last
        , first_value(lab.hemoglobin) over (partition by lab.visit_occurrence_id order by case when lab.hemoglobin is not null then 0 else 1 end asc, lab.rn desc) as hemoglobin_last
        , first_value(lab.creatinine) over (partition by lab.visit_occurrence_id order by case when lab.creatinine is not null then 0 else 1 end asc, lab.rn desc) as creatinine_last
        from """ + schemaName + """.labs_stg_1 lab
        ;
    """
...
def formatData(con, schemaName):
    ...
    dataDataMatrixQuery = """
        ...
        on vit_std.visit_occurrence_id = coh.visit_occurrence_id
        ...
        on lab_std.visit_occurrence_id = coh.visit_occurrence_id
        ...
        on dth.visit_occurrence_id = coh.visit_occurrence_id
        ;
    """
...
def imputeMissingData(con, schemaName):
    ...
    rawDf = dataDf.drop(['visit_occurrence_id', 'death_datetime'], axis = 1)
    knnImputer = KNNImputer()
    mfImputedData = knnImputer.fit_transform(rawDf)
    mfImputedDf = pd.DataFrame(mfImputedData, columns=rawDf.columns, index=rawDf.index)
    finalDf = mfImputedDf.join(dataDf[['visit_occurrence_id', 'death_datetime']])
...
def formatAll(con, vitalsBefore = 48, vitalsAfter = 48, labsBefore = 72, labsAfter = 72):
